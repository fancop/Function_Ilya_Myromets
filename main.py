...
def use_item():
    """
    Показать пронумерованые предметы
    Дать пользователю выбрать индекс этого предмета

    Тип: оружие или щи  -> тогда предмет экипируется
    Тип: продукт -> consume_item()

    Нужна функция, которая добавляет/убирает модификатор предмета к персонажу
    """
    pass
